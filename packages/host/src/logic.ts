...
import {
  createPregameCountdownMessage, createQuestionClosedMessage, createQuestionCountdownMessage,
  ...
  function runNext() {
    ...
    while (!current.done && running === true) {
        ...
        if (value.type === 'nextquestion') {
          ...
          gameInstance.broadcast(createQuestionGivenMessage(gameInstance.currentQuestion as QuestionType))
        }
