...
export const QuestionScreen = () => {
  ...
  return (
    <StonewallContainer>
      {state.current?.state === ScreenState.QuestionGiven ||
          ...
          <div css={{ display: 'flex' }}>
            ...
            <div css={{ flex: 'auto' }}>
              <input
                type="range"
                css={{ width: '100%' }}
                step={state.current.question.range.step}
                max={state.current.question.range.to}
                min={0}
                onMouseUp={(e) => client.send(createClientRangeAnswer(e))}
